...
import com.jyou.utils.BufferInfo;
import com.jyou.utils.ImageLoader;
...
public class AFRFace {
	...
	public static final String FD_SDKKEY ;
	public static final String FR_SDKKEY ;
    ...
    /**
     * 自定义静态代码块,读取properties,根据操作系统的不同初始化APPId等
     * Renxin
     * 2018-04-10
     */
    static {
        ...
        if (Platform.isWindows() == true){
            APPID = properties.getProperty("windows.APPID");
            FD_SDKKEY = properties.getProperty("windows.FD_SDKKEY");
            FR_SDKKEY = properties.getProperty("windows.FR_SDKKEY");
        }else{
            APPID = properties.getProperty("linux.APPID");
            FD_SDKKEY = properties.getProperty("linux.FD_SDKKEY");
            FR_SDKKEY = properties.getProperty("linux.FR_SDKKEY");
        }
     ...
     * @return
     */
   public   static Float doFaceRecognition(String faceUrl,String faceR){
        System.out.println("#####################################################");
        ...
        if (bUseRAWFile) {
            ...
            String filePathA = faceUrl;
            String filePathB = faceR;

            inputImgA = loadImage(filePathA);
            inputImgB = loadImage(filePathB);
        }
       float result = compareFaceSimilarity(hFDEngine, hFREngine, inputImgA, inputImgB);
       System.out.println(String.format("similarity between faceA and faceB is %f" ,result));
    ...
    public static float compareFaceSimilarity(Pointer hFDEngine, Pointer hFREngine, ASVLOFFSCREEN inputImgA, ASVLOFFSCREEN inputImgB) {
        ...
        //新增1------------------------------------------------------------------------------------------//
        //从AFR_FSDK_FACEMODEL 特征值对象中获取数据
     /*   try {
            byte[] bytes = faceFeatureB.toByteArray();
            String path1 = "D:/aa2/face.obj";
        ...
        //新增2------------------------------------------------------------------------------------------//
        //从AFR_FSDK_FACEMODEL 特征值对象中获取数据
        //AFR_FSDK_FACEMODEL.fromByteArray(byte)
        //end2-------------------------------------------------------------------------------------------//
